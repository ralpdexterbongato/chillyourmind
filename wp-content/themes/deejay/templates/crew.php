					...
					for ( $i = 1; $i < 9; $i++ ) {
						...
						if ( ! empty( $crewmember ) ) {
							...
							if ( count_user_posts( $crewmember->ID ) ) {
								echo '<a href="' . esc_url( get_author_posts_url( $crewmember->ID ) ) . '">' . $crewmember->display_name . '</a>';
							} elseif ( $crewmember->user_url ) {
									echo '<a href="' . esc_url( $crewmember->user_url ) . '">' . $crewmember->display_name . '</a>';
							} else {
								echo $crewmember->display_name;
							}
							echo '<br>';
